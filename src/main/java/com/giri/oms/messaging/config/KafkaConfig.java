 ...
 * package-info). order-events, customer-events, and, as of Phase 3's
 * completion, shipment-events are topics this service doesn't own
 * (oms-main, customer-service, and shipment-service, respectively) but
 * reads from as its own independent consumer group(s) — declaring them
...
public class KafkaConfig {
    ...
    // every one of this service's listened-to topics' own .DLT topics up
    // front.
    @Bean
    ...
    // shipment-service already declares this topic itself (it's both
    // producer and consumer on it) — this bean is the same
    // idempotent/defensive redeclaration as the two above, not this
    // service claiming ownership.
    @Bean
    NewTopic shipmentEventsTopic(KafkaAppProperties kafkaAppProperties) {
        return TopicBuilder.name(kafkaAppProperties.topics().shipmentEvents())
                .partitions(3)
                .replicas(1)
                .build();
    }
    ...
    @Bean
    NewTopic shipmentEventsDeadLetterTopic(KafkaAppProperties kafkaAppProperties) {
        return TopicBuilder.name(kafkaAppProperties.topics().shipmentEvents() + ".DLT")
                .partitions(3)
                .replicas(1)
                .build();
    }
     ...
     * ShipmentNotificationConsumer as of Phase 3's completion) — Spring
     * Boot's Kafka autoconfiguration wires any single CommonErrorHandler
     * bean into the default listener container factory automatically, so
     * every listener shares this one bean without any needing its own —
     * mirrors oms-main's/shipment-service's own kafkaErrorHandler bean
     * exactly. Retries a failure 3 times, 2 seconds apart, to ride out a
     * brief CustomerClient/OrderClient blip without holding up the
     * partition for too long; if still failing after that, republishes the
     * raw record to "<source-topic>.DLT" instead of blocking the partition
     * forever or silently dropping the message.
     */
    @Bean
